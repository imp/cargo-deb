...
fn process(
    ...
    // The block frees the large data_archive var early
    {
        ...
        let control_archive =
            control::generate_archive(&options, system_time, asset_hashes, listener)?;
        ...
        match data_compressed? {
            compress::Compressed::Gz(data) => {
                deb_contents.add_data("data.tar.gz", system_time, &data)?
            }
            compress::Compressed::Xz(data) => {
                deb_contents.add_data("data.tar.xz", system_time, &data)?
            }
        }
    }
