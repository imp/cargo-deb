...
fn run_cargo_deb_command_on_example_dir() {
    ...
    assert!(ddir
        .path()
        .join("usr/share/doc/example/changelog.gz")
        .exists());
...
fn run_cargo_deb_command_on_example_dir_with_variant() {
    let root = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").unwrap());
    let cmd_path = root.join(format!(
        "target/debug/cargo-deb{}",
    ...
    assert!(md5sums
        .contains("f4b165c5ea1f9ec1b87abd72845627fd  usr/share/doc/example-debug/copyright\n"));
    ...
    assert!(ddir
        .path()
        .join("usr/share/doc/example-debug/changelog.gz")
        .exists());
    assert!(ddir.path().join("usr/bin/example").exists());
}
